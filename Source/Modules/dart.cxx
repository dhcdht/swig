...
public:
  ...
  virtual int enumDeclaration(Node *n) {
	...
	if (proxy_flag && is_wrapping_class()) {
	  ...
	  String *filen = NewStringf("%s%s.dart", output_directory, symname);
	  ...
	  Printv(f_enum, typemapLookup(n, "dartimports", typemap_lookup_type, WARN_NONE), // Import statements
		 "\n", enum_code, "\n", NIL);
   ...
   * jniclasscode            - text (dart code) is copied verbatim to the intermediary class
  ...
  void addInterfaceNameAndUpcasts(SwigType *smart, String *interface_list, String *interface_upcasts, List *base_list, SwigType *c_classname) {
	...
	cptr_method_name = Copy(Getattr(attributes, "tmap:dartinterfacecode:cptrmethod"));
      }
      if (!cptr_method_name)
  ...
  //   }
    ...
    if (derived) {
      tm = typemapLookup(n, "dartdestruct_derived", typemap_lookup_type, WARN_NONE, attributes);
      destruct_methodname = Getattr(attributes, "tmap:dartdestruct_derived:methodname");
      destruct_methodmodifiers = Getattr(attributes, "tmap:dartdestruct_derived:methodmodifiers");
      destruct_parameters = Getattr(attributes, "tmap:dartdestruct_derived:parameters");
    } else {
      tm = typemapLookup(n, "dartdestruct", typemap_lookup_type, WARN_NONE, attributes);
      destruct_methodname = Getattr(attributes, "tmap:dartdestruct:methodname");
      destruct_methodmodifiers = Getattr(attributes, "tmap:dartdestruct:methodmodifiers");
      destruct_parameters = Getattr(attributes, "tmap:dartdestruct:parameters");
    }
    if (tm && *Char(tm)) {
      if (!destruct_methodname) {
	Swig_error(Getfile(n), Getline(n), "No methodname attribute defined in dartdestruct%s typemap for %s\n", (derived ? "_derived" : ""), proxy_class_name);
      }
      if (!destruct_methodmodifiers) {
	Swig_error(Getfile(n), Getline(n), "No methodmodifiers attribute defined in dartdestruct%s typemap for %s.\n", (derived ? "_derived" : ""), proxy_class_name);
      }
	...
	Printv(proxy_class_def, typemapLookup(n, "dartfinalize", typemap_lookup_type, WARN_NONE), NIL);
      }
      // delete method
  ...
  * ---------------------------------------------------------------------- */
  ...
  virtual int classHandler(Node *n) {
	...
	if (interface_name && !addInterfaceSymbol(interface_name, n, nspace))
      ...
      if (!has_outerclass) {
	String *output_directory = outputDirectory(nspace);
	String *filen = NewStringf("%s%s.dart", output_directory, proxy_class_name);
	...
	String *output_directory = outputDirectory(nspace);
	String *filen = NewStringf("%s%s.dart", output_directory, interface_name);
    ...
    if (proxy_flag) {
      ...
      Replaceall(interface_class_code, "$dartclassname", proxy_class_name);
      ...
      Replaceall(interface_class_code, "$dartclazzname", dartclazzname);
  ...
  //   }
	...
	if (prematureGarbageCollectionPreventionParameter(pt, p)) {
          String *pgcppname = Getattr(p, "tmap:dartin:pgcppname");
          if (pgcppname) {
            String *argname = Copy(pgcppname);
            Replaceall(argname, "$dartinput", arg);
  ...
  virtual int constructorHandler(Node *n) {
    ...
    int i;
    String *function_code = NewString("");
    String *helper_code = NewString(""); // Holds code for the constructor helper method generated only when the dartin typemap has code in the pre or post attributes
  ...
  //     }
	...
	p = Getattr(p, "tmap:in:next");
      ...
      /* Insert the dartconstruct typemap, doing the replacement for $directorconnect, as needed */
  ...
  bool substituteClassname(SwigType *pt, String *tm, bool jnidescriptor = false) {
    ...
    if (Strstr(tm, "$dartclassname")) {
      ...
      substituteClassnameSpecialVariable(classnametype, tm, "$dartclassname", jnidescriptor);
    ...
    if (Strstr(tm, "$dartinterfacename")) {
      SwigType *interfacenametype = Copy(strippedtype);
      substituteInterfacenameSpecialVariable(interfacenametype, tm, "$dartinterfacename", jnidescriptor, true);
      substitution_performed = true;
  ...
  void emitTypeWrapperClass(String *classname, SwigType *type) {
    ...
    String *filen = NewStringf("%s%s.dart", SWIG_output_directory(), classname);
  ...
  void addThrows(Node *n, const String *attribute, Node *parameter) {
    ...
    if (throws && Len(throws) > 0) {
      String *throws_list = Getattr(n, "dart:throwslist");
      if (!throws_list) {
	throws_list = NewList();
	Setattr(n, "dart:throwslist", throws_list);
      }
  ...
  void generateThrowsClause(Node *n, String *code) {
    ...
    List *throws_list = Getattr(n, "dart:throwslist");
    if (throws_list) {
  ...
  String *prematureGarbageCollectionPreventionParameter(SwigType *t, Parm *p) {
	...
	if (!GetFlag(p, "tmap:jtype:nopgcpp") && !nopgcpp_flag) {
	  String *interface_name = getInterfaceName(t, true);
          pgcpp_dart_type = interface_name ? interface_name : getProxyName(t);
          if (!pgcpp_dart_type) {
  ...
  void emitDirectorExtraMethods(Node *n) {
    ...
    Printf(code_wrap->code, "    director->swig_dart_change_ownership(jenv, jself, jtake_or_release ? true : false);\n");
    Printf(code_wrap->code, "  }\n");
    Printf(code_wrap->code, "}\n");
   ...
   *
   * Replace $packagepath using the dartpackage typemap associated with passed
  ...
  void substitutePackagePath(String *text, Parm *p) {
    ...
    if (p)
      pkg_path = Swig_typemap_lookup("dartpackage", p, "", 0);
  ...
  int classDirectorMethod(Node *n, Node *parent, String *super) {
	...
	if ((desc_tm = Swig_typemap_lookup("directorin", tp, "", 0))
	  ...
	  if ((tm = Getattr(p, "tmap:jtype"))) {
	    String *din = Copy(Getattr(p, "tmap:dartdirectorin"));
            addThrows(n, "tmap:dartdirectorin", p);
    ...
    if (!is_void) {
      if ((tm = Swig_typemap_lookup("dartdirectorout", n, "", 0))) {
        addThrows(n, "tmap:dartdirectorout", n);
	substituteClassname(returntype, tm);
	Replaceall(tm, "$dartcall", upcall);
  ...
  void directorExceptHandler(Node *n, ParmList *throw_parm_list, Wrapper *w) {
	...
	for (Parm *p = throw_parm_list; p; p = nextSibling(p)) {
	  ...
	  if (tm) {
	    // replace $packagepath/$dartclassname
